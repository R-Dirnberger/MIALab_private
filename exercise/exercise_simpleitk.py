...
def register_images(img, label_img, atlas_img):
    ...
    registered_img = sitk.Resample(image1=img, referenceImage=atlas_img, transform=transform,
    ...
    registered_label = sitk.Resample(image1=label_img, referenceImage=atlas_img, transform=transform, interpolator=sitk.sitkNearestNeighbor,
...
def preprocess_rescale_numpy(np_img, new_min_val, new_max_val):
    ...
    offset = new_min_val
    rescaled_np_img = np_img * scaling_factor + offset
...
def postprocess_largest_component(label_img):
    ...
    relabeled_components = sitk.RelabelComponent(image1=connected_components, sortByObjectSize=True)
